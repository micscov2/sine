...
//addEntry(50);
updateMethod();
//printElementsOfTable('personalTableM');
...
function initProcess() 
{
	...
	$record = "CREATE TABLE personalTableM (name INT NOT NULL)";
	if (! $GLOBALS['con'] -> query("SELECT *FROM personalTableM")) {
		if ($GLOBALS['con'] -> query($record) == FALSE) {
...
function addEntry($userToBeAdded) 
{
	...
	$record = "INSERT INTO personalTableM (name) VALUES (40)";
	if ($GLOBALS['con'] -> query($record) == FALSE) {
